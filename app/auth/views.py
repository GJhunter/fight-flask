...
@auth.before_app_request#这个修饰器保证这个钩子能够在全局中使用
...
def login():
	...
	if form.validate_on_submit():
		...
		if user is not None and user.verify_password(form.password.data):
			login_user(user,form.remember_me.data)#flask-login中的login_user函数会在用户会话中将用户标记为已登录，第二个参数用来是否记住用户
			return redirect(request.args.get('next') or url_for('main.index'))#request.args.get('next')可以用来读取登录页面前的那个页面，若没有则重定向到主页
...
@login_required
def logout():
	logout_user()#logout_user函数会删除并重设用户会话
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()#因为这里提交数据库以后才能赋予新用户id值，确认令牌需要id，所以不能延后提交
...
def unconfirmed():
	if current_user.is_anonymous() or current_user.confirmed:#普通用户current_user.is_anonymous() 返回false
		return redirect(url_for('main.index'))
...
def password_reset(token):
	if not current_user.is_anonymous:#已经登录的用户is_anonymous为True,这个是为了防止已经登录的用户点击链接的
		return redirect(url_for('main.index'))
